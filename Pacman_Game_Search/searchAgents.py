...
def cornersHeuristic(state, problem):
    ...
    unvisitedCorners = [corners[i] for i in range(len(corners)) if not visited[i]]
    heuristic = 0

    while unvisitedCorners:
        distance, corner = min([(util.manhattanDistance(position, corner), corner) for corner in unvisitedCorners])
        heuristic += distance
        position = corner
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
         # Create a problem instance around the current state
        problem = AnyFoodSearchProblem(gameState)

        # Use a search algorithm to solve the problem
        # In this case, breadth-first search is a good choice for finding the shortest path
